    ...
    data(){
        const checkUsername=(rule,value,callback)=>{
            ...
            if(!rs){
                ...
                return callback(new Error("用户名不能包含特俗字符"));
            }
            return callback();
        }
        return{
            ...
            rules:{
                username:[
                    ...
                    {min:3,max:20,message:"用户名长度不能大于20或者小于3",trigger:'change'},
                    {validator:checkUsername,trigger:'input',trigger:'change'}
                ],
                password:[
                         {required:true,message:"请填写密码",trigger:'blur'}, 
                         {min:6,max:20,message:"密码长度不能大于20且不小于6",trigger:'blur'}
                ]
            }
       }
    },
